...
function updateDOM(state, bookObj){
    let bookDOM = bookObj.domtext;

    switch(state){
        case 'read':
            bookDOM.textContent = bookObj.info;
            break;
        case 'remove':
            break;
        default:
...
let readButtons = document.querySelectorAll('.read-book');
readButtons.forEach((btn, index) => btn.addEventListener('click',()=>{
    ...
    updateDOM('read', myLibrary[index]);
